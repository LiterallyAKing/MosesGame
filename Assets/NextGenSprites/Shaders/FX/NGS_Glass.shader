	...
	SubShader {
		...
		Pass {
			...
			float4 frag(VertexOutput i) : COLOR {
				...
				half Colorcombined = (_MainTex_var.a*_Color.a*i.vertexColor.a);
				half3 diffuseColor = ((_MainTex_var.rgb*_Color.rgb*i.vertexColor.rgb)*Colorcombined);
				half3 diffuse = (directDiffuse + indirectDiffuse) * diffuseColor;

				// Emission
				half3 emissive = (_EmissionStrength*_MainTex_var.rgb);
		...
		Pass {
			...
			float4 frag(VertexOutput i) : COLOR {
				...
				// Final Color
				return fixed4(diffuse * (Colorcombined*_OpacitySprite),0);
			}
			ENDCG
		}
	}

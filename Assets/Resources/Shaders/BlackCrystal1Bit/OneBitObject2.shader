Shader "Custom/OneBitObject2" {
    ...
    _Spherical ("Spherical", Float) = 0.0
	}
	SubShader {
...
v2f_surf vert_surf (appdata_full v) {
  ...
  float4 vnorm;
  if(_Spherical) vnorm = float4(cameraDir*-1.0, 0.0);
  else vnorm = float4(v.normal, 0.0);
...
fixed4 frag_surf (v2f_surf IN) : SV_Target {
  ...
  return fixed4(IN.color.r, c.g/2.0, IN.color.b, 0);
}
...
fixed4 frag_surf (v2f_surf IN) : SV_Target {
  ...
  return fixed4(IN.color.r, c.g, IN.color.b, 0);
}
...
fixed4 frag_surf (v2f_surf IN) : SV_Target {
  ...
  fixed4 r = UnityMetaFragment(metaIN);
  return fixed4(r.rgb, 0);
}
